...
- 每个交易日必须先完成 A 股和港股的逐票复盘，再生成或更新总的每日预测汇总台账；不得在复盘尚未完成时把记录标记为已复盘。
- 逐票复盘至少填写：目标日期、市场、代码、名称、收盘价、涨跌幅、是否触发、是否失效、复盘结果和复盘备注。
- 复盘完成后，将当日结果追加到历史明细，并同步更新每日统计、累计统计及总汇总表。
- 历史记录只允许追加或更正，不得用当日数据覆盖旧日期；更正旧数据时应保留可追溯的日期和说明。
- A 股与港股分别保留原始台账，同时维护一份合并总台账；合并时按“目标日期 + 市场 + 代码”检查重复记录。
- 每日工作结束前核对：当日预测数 = 当日复盘数；若数据源未齐，应保留为“待复盘”并注明原因。
- 当日复盘和汇总完成后，必须继续预测下一交易日的 A 股与港股票；两类市场分别输出预测清单、预测逻辑、触发条件、失效条件和风险提示。
- 次日预测完成后，立即把每只预测票追加到对应市场的预测台账，复盘字段统一标记为“待复盘”，确保下一交易日可逐票追踪。
- 每日生成的报告、表格和快照使用日期命名，适合入库的数据纳入 GitHub 日常备份。
